...
| 검증 | 도구 | 대상 | 통과 기준 | 차단? | 비고 |
|------|------|------|----------|-------|------|
| Python Type | `mypy app/` | staged `*.py` | warning 출력 | **No (경고만)** | 기존 코드 타입 불완전 → 점진적 개선 |
| TypeScript | `npx tsc --noEmit` | `frontend/src/` | exit 0 | **Yes (차단)** | TypeScript는 엄격 |

**원칙**:
- Python mypy: **경고만 출력, 커밋 차단하지 않음** (기존 코드에 타입 힌트 불완전)
- TypeScript: 0-tolerance (tsc 에러 = 차단)
- `/ship` 단계에서 full mypy 체크 시행 (pre-commit에서는 warn only)
