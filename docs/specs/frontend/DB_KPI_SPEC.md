...
### 4.5 Metrics Timeline / ASH Heatmap X축 시간 정책

시간 범위 프리셋에 따라 X축 레이블 포맷, 포인트 간격, 레이블 수가 적응적으로 변경됩니다.
**이 정책은 Metrics Timeline과 ASH Heatmap 모두에 공통 적용됩니다.**

#### 포인트 간격 정책 (핵심)

| 프리셋 | 전체 시간 | 이상적 포인트 간격 | X축 레이블 수 | 레이블 간격 예시 |
|--------|----------|------------------|-------------|----------------|
| **5m** | 300초 | 5초 → 60포인트 | ~12 | 25초마다 |
| **15m** | 900초 | 10초 → 90포인트 | ~10 | 1.5분마다 |
| **30m** | 1800초 | 15초 → 120포인트 | ~10 | 3분마다 |
| **1h** | 3600초 | 30초 → 120포인트 | ~12 | 5분마다 |
| **6h** | 21600초 | 5분 → 72포인트 | ~12 | 30분마다 |
| **24h** | 86400초 | 20분 → 72포인트 | ~12 | 2시간마다 |
| **7d** | 604800초 | 2시간 → 84포인트 | ~12 | 12시간마다 |

#### X축 포맷 + 차트 설정

| 프리셋 | X축 포맷 | 최대 포인트 | 레이블 간격 | 회전 | 해상도 표시 |
|--------|----------|-----------|-----------|------|-----------|
| **5m** | `HH:mm:ss` | 60 | 매 5번째 | 0° | 5초 (1초 원본) |
| **15m** | `HH:mm:ss` | 90 | 매 9번째 | 0° | 10초 (1초 원본) |
| **30m** | `HH:mm:ss` | 120 | 매 12번째 | 0° | 15초 (1초 원본) |
| **1h** | `HH:mm` | 120 | 매 10번째 | 0° | 30초 (10초 집계) |
| **6h** | `HH:mm` | 72 | 매 6번째 | 0° | 5분 (1분 집계) |
| **24h** | `HH:mm` | 72 | 매 6번째 | 35° | 20분 (1분 집계) |
| **7d** | `MM/dd HH` | 84 | 매 7번째 | 35° | 2시간 (1시간 집계) |

#### X축 레이블 규칙

- **목표**: 화면에 **10~12개 레이블**이 보이도록 간격 자동 조정
- **짧은 범위 (5m~6h)**: 수평 레이블
- **긴 범위 (24h~7d)**: 35° 회전, 차트 하단 패딩 48px로 증가
- **7d**: `MM/dd HH`로 날짜 구분 우선
- **프리셋 버튼**: "5분 (1초)", "15분 (1초)" 등 해상도 동시 표시
