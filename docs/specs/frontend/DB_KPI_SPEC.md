...
| 프리셋 | 시간 범위 | X축 포맷 | 다운샘플링 | 예시 |
|--------|----------|----------|-----------|------|
| **15m** | 15분 | `HH:mm:ss` (초 단위) | 없음 (원본 1초) | 14:30:15 |
| **1h** | 1시간 | `HH:mm` (분 단위) | 10초 평균 | 14:30 |
| **6h** | 6시간 | `HH:mm` (분 단위) | 1분 평균 | 14:30 |
| **24h** | 24시간 | `HH:mm` (시:분) | 5분 평균 | 14:30 |
| **7d** | 7일 | `MM/DD HH:mm` (날짜+시간) | 1시간 평균 | 03/25 14:00 |
...
- **게이지 메트릭** (numbackends): 버킷 내 마지막 값 (또는 평균)
- **카운터 메트릭** (xact_commit): 버킷 경계 값의 delta/s
- 차트 포인트 수 목표: 100~200개 (가독성 최적)
